...
extern uint8_t _binary_tty_lat9_16_psf_start;
extern uint8_t _binary_tty_lat9_16_psf_end;
extern struct framebuffer_info_common fb_info;

#define PIXEL uint32_t

struct psf_font get_psf_info() {
  return *((struct psf_font*)&_binary_tty_lat9_16_psf_start);
}

// cx and cy should probably be size_t
void psf_putchar_at(uint16_t c, uint32_t cx, uint32_t cy, uint32_t fg, uint32_t bg) {

  struct psf_font* font = (struct psf_font*)&_binary_tty_lat9_16_psf_start;
  uint8_t* glyphs_start = (uint8_t*)&_binary_tty_lat9_16_psf_start + font->headersize;
